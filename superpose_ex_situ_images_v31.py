...
class mainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.title = 'Superpose ex-situ images v31'
...
class MyTableWidget(QWidget):        
    ...
    def __init__(self, parent):   
        ...
        self.LbLatRot = QLabel('Rotate by: ')  # label (passif)
        self.EdLatRot = QLineEdit('-110')  # edit
        self.LbLatRotUnit = QLabel('° around center ')  # label (passif)
        
        self.LbLatXShi = QLabel('Shift hor. by: ')  # label (passif)
        self.EdLatXShi = QLineEdit('4')  # edit
        self.LbLatXShiUnit = QLabel('pixels ')  # label (passif)
        
        self.LbLatYShi = QLabel('shift ver. by: ')  # label (passif)
        self.EdLatYShi = QLineEdit('5')  # edit
        self.LbLatYShiUnit = QLabel('pixels ')  # label (passif)
        
        self.BtUpdateTransfo = QPushButton('UPDATE')
        ...
        ### For exporting the pixel cloud
        self.BtPiCloExp = QPushButton('Export to Excel')
        self.BtPiCloExp.clicked.connect(self.Export_PixelCloud)
        ...
        grid4.addWidget(self.canvasPiCloud, 3, 7, 12, 4)
        grid4.addWidget(self.toolbarPiCloud, 16, 7, 1, 4)
        
        grid4.addWidget(self.BtPiCloUpd, 18,7,1,2)
        grid4.addWidget(self.BtPiCloExp, 18,9,1,2)
    ...
    def rdbt3Tog(self,newChoice):
        ...
        # global absLevs # if the contour plot is used
        ...
        if newChoice == 0 :
            axIMdi.imshow(imDisplay, cmap='inferno', vmin=vmini, vmax=vmaxi  )
        elif newChoice == 1:
#            # make contour plot with same orientation as imshow(origin=upper)
#            imDisplay = imDisplay[::-1,:] # flips the lowest line up... 
#            X, Y = np.meshgrid(range(imDisplay.shape[1]), range(imDisplay.shape[0]))
#            
#            if self.ckAbsLevs.isChecked():
#                levs = absLevs 
#            else:
#                levs = imDisplay.max() * np.array(
#                    [0, 0.0250, 0.05, 0.075, 0.10, 0.15])
#                absLevs = levs
#            print('----')
#            print(levs) 
#            cax = axIMdi.contour(X, Y, imDisplay, levs, linewidths=0.5, colors='k')
#            cax = axIMdi.contourf(X, Y, imDisplay, levs, cmap=cm.jet)        
#            axIMdi.set_aspect('equal', adjustable='box')
                
            print('----')
            print(floName2[-25:-4])
            print('Max = ' + str(imDisplay.max()))
            print('Sum(En) = ' + str(imDisplay.sum()))
            
            cax = axIMdi.imshow(imDisplay, cmap='inferno', vmin=vmini, vmax=vmaxi )  
            # self.figureIMdi.colorbar(cax, orientation='vertical',spacing='regular')   
        else:
            vmini = imDisplay.min()
            vmaxi = imDisplay.max()
            axIMdi.imshow(imDisplay, cmap='inferno', vmin=vmini, vmax=vmaxi  )
        ...
        message = 'imDisplay is of type ' + str(
                imDisplay.dtype)+ ' and shape '+ str(imDisplay.shape)
        print(message)
        message = ('Limits of ndarray: min: {:3.2f}, max:' +
                      '{:3.2f}').format(imDisplay.min(),imDisplay.max()) 
        print(message)
    ...
    def Read_Tra_Params(self):
        ...
        if self.ckLatXFlip.isChecked():
            ...
            Pfli = -1
        Pangle = float(self.EdLatRot.text()) # degrees
        PxOffset = float(self.EdLatXShi.text()) # pixels to the right
        PyOffset = -1 * float(self.EdLatYShi.text()) # pixels up
    ...
    def makeImFlo(self):
        global imFlo
        # prepare imFlo having 2 times the size of imRefOri filled with zeros
        imFlo = np.zeros((2*imRefOri.shape[0],2*imRefOri.shape[1]))
        ...
        # and keep 2 times the size of imRefOri 
        # calc max
        ...
        imFlo = imFlo/imFlo.max()
        message = 'makeImFlo: imFlo is of type ' + str(
                imFlo.dtype)+ ' and shape '+ str(imFlo.shape)
        print(message)
        return
...
#        global lry
        ...
        if TabNo == 2:
            ...
            # normalization for the fit tab has been moved to crop_Ref and makeimFlo
...
#            lry = -50            
            ...
            ps = self.Read_Tra_Params()  # makes a tuple (no items assignment possible)
            ps = np.array(ps) # tranforms to array
            # make sure there are no z modifications
            ps[0] = 0 # Do not change offset here (do this in only tabs im1 and im2)
            ps[1] = 1 # Do not change scaling here (do this later)
            self.Calc_New_Float(*ps)  # contains: imFlo = imFloOri.copy()        
    ...
    def Update_PixelCloud(self):
        global x # array of x coordinates of the pixel cloud
        global y # array of y coordinates of the pixel cloud
...
#        # plot y-axis in image style (0 on top)
    ...
    def Export_PixelCloud(self):
        # simple version
        mywb = xl.Workbook() # workbook
        ws1 = mywb.active # worksheet
        ws1.title = 'pixel cloud'
        
        # describe the origin of the data
        ws1.cell(row=1, column=4).value = 'X-data: '
        ws1.cell(row=2, column=4).value = 'Y-data: '
        XChoice = self.rdbtgroupX.checkedId()
        if (XChoice == 0 and flo =='1') or (XChoice == 1 and flo =='2'):
            ws1.cell(row=1, column=5).value = floName2
            ws1.cell(row=2, column=5).value = refName1
        else :
            ws1.cell(row=2, column=5).value = floName2
            ws1.cell(row=1, column=5).value = refName1
        
        ws1.cell(row=1, column=1).value = 'x'
        ws1.cell(row=1, column=2).value = 'y'
        for count in range(len(x)):
            ws1.cell(row=count+2, column=1).value = x[count] 
            ws1.cell(row=count+2, column=2).value = y[count]
    ...
    def Make_fit(self):
        ...
        print(' Pangle = {:1.3e}, '.format(p0[3]))
        print(' PxOffset = {:1.3e}, '.format(p0[4]))
        print(' PyOffset = {:1.3e}, '.format(p0[5]))

        # split off the fixed parameters ant transfert to pFix
        pFix = np.array(p0[0:3])  # do not vary the first 3 parameters
        print(' pFix shape : ', pFix.shape) #
        
        # transfer the parameters to vary in pVar0
        pVar0 = np.array(p0[3:]) # copy the ranges of params
        print(' pVar0 shape : ', pVar0.shape) # 3,1
    ...
    def crop_Flo_to_Ref(self):
        ...
        # imFlo is larger than imRefOri (2*) so xDif and yDif sont >0
        ...
        message = 'crop_Flo_to_Ref: imFloCut is of type ' + str(
    ...
    def crop_Ref(self):
        ...
        imRefCut = imRefOri.copy()/imRefOri.max()
        ...
        message = 'crop_Ref: imRefCut is of type ' + str(
...
#        global Im2lry
    ...
    def showMask(self,btID):
        ...
        vmini = MaskedImage.min()
        vmaxi = MaskedImage.max()
        ...
        axIMdi = self.figureIMdi.add_subplot(111)
        axIMdi.axis("off")
        cax = axIMdi.imshow(MaskedImage, cmap='inferno',vmin=vmini, vmax=vmaxi)
        # common color maps: gray, hot, hsv, inferno, gist_ncar
